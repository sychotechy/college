...
void transpose(int matrix[][3], int matrixTranspose[][3]){
	...
	for(i=0;i<matrixTranspose[0][2]+1;i++){
		for(j=0;j<3;j++){
...
int main(){
	...
	int count=0;
	for (int i = 1; i < matrixTranspose[0][2]+1 + 1; i++){
		for (int j = 1; j < matrixTranspose[0][2]+1; j++){
			if(matrix[i][2] == matrixTranspose[j][2]){
				if(matrix[i][0]==matrixTranspose[j][1] && matrix[i][1]==matrixTranspose[j][0]){
					count++;
				} 
			}
		}
	}
	if(count==matrixTranspose[0][2]){
    	printf("Symmetric\n");
    } 
	else{
